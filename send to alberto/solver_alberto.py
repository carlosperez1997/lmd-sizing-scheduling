...
class Solver:
    ...
    def __build_flexible_model(self) -> None:
        ...
        self.m.addConstrs((
            ...
            for region in self.i.regions
            for theta in self.i.periods
            # if theta < self.i.n_periods - 1 - shift_len # only considers shifts before the 3rds period
            if theta < self.i.n_periods + 1 - shift_len # considers shifts before the 5th period
        ), name='fix_region_n_couriers_in_shift')
    ...
    def __build_partflex_model(self) -> None:
        ...
        #w_idx = range(self.i.n_periods - shift_len - 1) # only creates for shifts [0, 1, 2]
        w_idx = range(self.i.n_periods - shift_len + 1) # creates w for periods [0, 1, 2, 4]
