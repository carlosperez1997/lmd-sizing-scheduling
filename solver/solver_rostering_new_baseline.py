...
class Solver:
    ...
    def __build_baseline_model(self) -> None:
        ...
        #w decision variable (e,d)
        self.w = {(employee, day): self.m.addVar(vtype=GRB.BINARY, name='w') 
                for region in self.i.regions 
                for employee in self.i.employees[region]
                for day in self.i.days}
        ...
        self.m.addConstrs((
            ...
            for day in self.i.days
            for theta in self.i.periods[day]
        ), name='baseline_works_a_day')

        #constraint - ensuring employee will have at least one rest day a week
        self.m.addConstrs((
            sum(self.w[(employee, day)]
                for day in self.i.days
                )
            <= N_WORK_DAYS
                for region in self.i.regions
                for employee in self.i.employees[region]
        ), name = 'baseline_n_rest_day_per_week')
...
def run_baseline_solver_output(model, instance_file_weekday, shift_file_weekday, instance_file_weekend, shift_file_weekend, workforce_dict, outsourcing_cost_multiplier, regional_multiplier, global_multiplier, h_min, h_max, max_n_diff, max_n_shifts=None, expand_workforce_to_regions=None):
    args = Namespace(
        model=model,
        instance_file_weekday=instance_file_weekday,
        shift_file_weekday = shift_file_weekday,
